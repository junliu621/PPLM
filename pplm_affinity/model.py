...
class PPLM_Affinity(nn.Module):
    def __init__(self, pplm_param):
        ...
        self.pplm_param = pplm_param
        self.alphabet = Alphabet.from_architecture()
        self.batch_converter = self.alphabet.get_batch_converter()
        self.model = PPLM(
            num_layers=self.pplm_param['encoder_layers'],
            embed_dim=self.pplm_param['encoder_embed_dim'],
            attention_heads=self.pplm_param['encoder_attention_heads'],
            token_dropout=False,
    ...
    def forward(self, seqA, seqB, device):
        ...
        max_pooling_embed = torch.max(embed, dim=0, keepdim=False).values
